...
Go to the OpenShift Console.  Make sure and change your view from 'Developer' to 'Administrator.
...
== Run the pipeline
To run the pipeline, start by downloading the `process_claims.yaml` file locally to your pc. +

Then go to the 'Red Hat OpenShift Data Science' dashboard.  Select your Data Science project, scroll down until you see the 'Data Connections' service, and press Import Pipeline.
...
== Check the results
After the pipeline has finished running, you can go to the app and take a look at the claims.  You will see that claim 3 is now processed.

[.bordershadow]
image::05/05-process-claim3-app.jpg[claim3processed]

Click on claim3.  Instead of just a long body, you will now see a summary, a location field, an accident time field, and a sentiment field. You can also see that we have new image(s) which have bounding boxes where the damage is.

[.bordershadow]
image::05/05-processed-claim.jpg[claim3processe]